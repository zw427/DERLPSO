...
class SDE_Models(DE_Models):
    ...
    def _solve_sde(self, params_dict: dict, y0: np.ndarray, t_eval: np.ndarray, seed: int = None) -> np.ndarray:
        if seed is not None:
            np.random.seed(seed)
        ...
        corr_matrix = self.equation.correlation_matrix if self.equation.correlation_matrix is not None else np.eye(dim)
        ...
        if dim == 1:
            ...
            for i in range(n_steps - 1):
                ...
                diffusion = np.atleast_2d(diffusion_func(x[i, :], t_eval[i], params_dict))
    ...
    def simulate(self, num_data: int, parameter: Parameter, t_span: list, point: int, seed: int = None) -> dict:
        ...
        for _ in range(num_data):
            ...
            data = self._solve_sde(params_dict, self.equation.initial_conditions, time_points)
            data_list.append(data)
            param_list.append(param_values)
            time_list.append(time_points)
        return {'data': np.stack(data_list), 'param': np.stack(param_list), 'time': np.stack(time_list)}

    def evaluate(self, estimator: Estimator, test_set: dict, train_set: dict = None, seed: int = None) -> dict:
        ...
        for i in range(data.shape[0]):
            ...
            prediction_data = self._solve_sde(pred_dict, self.equation.initial_conditions, time[i], seed=42)
    ...
    def pprint(self, output: dict, estimator_name: str, file: str = None):
        ...
        '''
        if file:
            with open(file, 'a', encoding='utf-8') as f: f.write(txt)
        else:
            print(txt)