...
from pde_equations import (
    create_heat_function,
    create_transient_function,
    create_helmholtz_function,
)
...
heat_α = heat_model.simulate(
    NUM_DATA, mu=0.5, sigma=0.5, lower=0.0001, upper=1.0, seed=SEED
)
...
transient_D = transient_model.simulate(
    NUM_DATA, mu=0.5, sigma=0.5, lower=0.0001, upper=1.0, seed=SEED + 1
)
transient_v = transient_model.simulate(
    NUM_DATA, mu=0.5, sigma=0.5, lower=0.0001, upper=1.0, seed=SEED + 2
)
data = [transient.f()([D, v]) for D, v in zip(transient_D, transient_v)]

test_set = {
    "param": np.array(list(zip(transient_D, transient_v))),
    "data": np.array(data),
}
...
helmholtz_λ = helmholtz_model.simulate(
    NUM_DATA, mu=0.5, sigma=0.5, lower=0.0001, upper=1.0, seed=SEED + 3
)
