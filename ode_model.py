...
class ODE_Models(DE_Models):
    ...
    def integrate(
        self, args: np.ndarray, y0: np.ndarray, t_eval: np.ndarray
    ) -> np.ndarray:
        """
        Integrate the ODE system defined by the equation with given parameters.
        """
        return scipy.integrate.odeint(
            self.equation.f(), y0, t_eval, args=(args,), tfirst=self.equation.t_first
        )
    ...
    def parameter_est(
        self, estimator: Estimator, data: np.ndarray, time: np.ndarray, seed: int = None
    ) -> np.ndarray:
        """
    ...
    def evaluate(
        ...
        assert (
            set(["data", "time", "param"]).issubset(set(train_set.keys()))
            if train_set
            else None
        )
        ...
        for i in range(data.shape[0]):
            prediction_data = self.integrate(
                prediction[i].squeeze(), data[i, 0], time[i]
            )
