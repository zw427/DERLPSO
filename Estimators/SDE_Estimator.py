...
class SDE_Estimator(Estimator):
    def __init__(self, func: SDE_Equation, particle_num=100, max_iter=200, grid_resolution=5):
    ...
    def _sde_prediction_concatenated(self, params_dict: dict, time_points: np.ndarray, seed_base: int) -> np.ndarray:
        sim_data_list = [
            self.sde_model._solve_sde(params_dict, self.func.initial_conditions, time_points, seed=seed_base + i) 
        ...
        if initial_conditions_np.size == 1:
            ...
            return np.concatenate([np.concatenate([traj[:, dim] for traj in sim_data_list]) for dim in range(dimension)])

    def _fitness_function(self, parameters: np.ndarray, target_data: np.ndarray, time_points: np.ndarray) -> float:
        param_names = list(self.func.param_bounds.keys())
        params_dict = dict(zip(param_names, parameters))
        try:
            predicted_data = self._sde_prediction_concatenated(params_dict, time_points, seed_base=42)
            if predicted_data.shape != target_data.shape: return float('inf')
            residuals = target_data - predicted_data
            if len(residuals) < 4: return np.mean(residuals**2)
    ...
    def _pso_optimizer(self, target_data: np.ndarray, time_points: np.ndarray, fixed_params: dict) -> tuple:
        opt_param_names = [name for name in self.func.param_bounds if name not in fixed_params]
        if not opt_param_names: return fixed_params, float('inf')
        ...
        for i in range(self.particleNum):
            ...
            pFit[i] = self._fitness_function(np.array(param_array), target_data, time_points)
            if pFit[i] < gFit:
        ...
        for iteration in range(self.maxIter):
            ...
            for i in range(self.particleNum):
                ...
                params_dict = {**fixed_params, **dict(zip(opt_param_names, X[i]))}
                param_array = [params_dict[name] for name in self.func.param_bounds.keys()]
                fitness = self._fitness_function(np.array(param_array), target_data, time_points)
    ...
    def train(self, train_data, seed=None): pass

    def predict(self, data: np.ndarray, time: np.ndarray, seed: int = None) -> np.ndarray:
        ...
        for i in range(num_samples):
            ...
            if not noise_param_names:
                ...
                noise_grids = {name: np.linspace(*self.func.param_bounds[name], self.grid_resolution) for name in noise_param_names}
                grid_combinations = [dict(zip(noise_grids.keys(), combo)) for combo in itertools.product(*noise_grids.values())]
                best_fitness, best_params_dict = float('inf'), None
                for fixed_params in tqdm(grid_combinations, desc=f"Grid Search (Sample {i+1})"):
                    params_dict, fitness = self._pso_optimizer(sample_data, sample_time, fixed_params)
                    if fitness < best_fitness:
                        best_fitness, best_params_dict = fitness, params_dict
            all_predictions[i, :] = [best_params_dict[name] for name in param_names_ordered]
