...
from Estimators.ml_model_components.encoder_decoder import (
    VAE_ODE_RNN,
    VAE_RNN,
    RNN,
    ODE_RNN,
...
from Estimators.ml_model_components.parameter_estimators import (
    VAE,
    EncoderDecoder,
    Base,
)
...
def create_model(
    ...
    if model_type == "vae":
        ...
        if net_configs["encoder"]["type"] == "ODE":
            ...
            ode_func = ODEFunc(
                input_dim=latent_dim, ode_func_net=ode_func_net, device=device
            ).to(device)

            diffeq_solver = DiffeqSolver(
                ode_func, "euler", odeint_rtol=1e-3, odeint_atol=1e-5, device=device
            )
        ...
        if net_configs["decoder"]["type"] == "ODE":
            ...
            ode_func = ODEFunc(
                input_dim=decoder_input_dim, ode_func_net=ode_func_net, device=device
            ).to(device)
        ...
        diffeq_solver = DiffeqSolver(
            ode_func, "euler", odeint_rtol=1e-6, odeint_atol=1e-8, device=device
        )
        ...
        raise ValueError(
            f"Unknown model type: {model_type}. Supported types: 'vae', 'ode_rnn', 'rnn', 'mlp'"
        )
