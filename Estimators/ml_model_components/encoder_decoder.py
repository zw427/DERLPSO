...
class RNN(nn.Module):
    ...
    def __init__(
        ...
        if GRU_update is None:
            self.GRU_update = GRUUnitOriginal(
                latent_dim, input_dim, n_units=n_GRUUnits
            ).to(device)
        else:
...
class ODE_RNN(nn.Module):
    ...
    def run_odernn(self, data: torch.Tensor, time_steps: torch.Tensor):
        ...
        # Initialize latent state
        prev_y = torch.zeros((n_traj, self.latent_dim), dtype=torch.float64).to(
            self.device
        )
        ...
        for i in range(1, len(time_steps)):
            if abs(prev_t - t_i) < minimum_step:
                ...
                time_points = linspace_vector(prev_t, t_i, n_intermediate_tp).to(
                    self.device
                )
            ...
            if torch.mean(ode_sol[:, :, 0] - prev_y) >= 0.001:
                Exception(
                    f"Error: first point of the ODE is not equal to initial value {torch.mean(ode_sol[:, 0, :] - prev_y)}"
                )
...
class VAE_ODE_RNN(nn.Module):
    ...
    def __init__(
        ...
        if GRU_update is None:
            self.GRU_update = GRUUnit(latent_dim, input_dim, n_units=n_GRUUnits).to(
                device
            )
        else:
            self.GRU_update = GRU_update
...
class VAE_RNN(nn.Module):
    ...
    def run_odernn(self, data: torch.Tensor, time_steps: torch.Tensor):
        ...
        prev_logvar = torch.zeros((n_traj, self.latent_dim), dtype=torch.float64).to(
            self.device
        )
...
class DecoderODE_RNN(nn.Module):
    ...
    def __init__(
        ...
        if GRU_update is None:
            self.GRU_update = GRUUnitOriginal(
                input_dim_param, dim_of_data, n_units=n_GRUUnits
            ).to(device)
        else:
            self.GRU_update = GRU_update
    ...
    def forward(
        self, data: torch.Tensor, time_steps: torch.Tensor, prior: torch.Tensor
    ):
        assert not torch.isnan(data).any()
        assert not torch.isnan(time_steps).any()
    ...
    def run_odernn(
        self, data: torch.Tensor, time_steps: torch.Tensor, prior: torch.Tensor = None
    ):
