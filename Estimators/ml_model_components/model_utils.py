...
def activaton_func(nonlinearity: str) -> nn.Module:
    ...
    if nonlinearity.lower() == "relu":
        ...
        raise ValueError(
            f'Unknown nonlinearity "{nonlinearity}". Accepting relu, leaky, tanh, or elu only.'
        )
...
def build_modules(hidden_dims: List[int], nonlinearity: str, normalize: bool = False):
    ...
    if len(hidden_dims) < 2:
        raise ValueError(
            "hidden_dims must contain at least input and output dimensions"
        )
    ...
    for i in range(len(hidden_dims) - 1):
        ...
        if i < len(hidden_dims) - 2:  # Not the output layer
            ...
            modules.append(nn.Sequential(linear_layer, activaton_func(nonlinearity)))
...
def linspace_vector(start: torch.Tensor, end: torch.Tensor, n_points: int):
    ...
    if size == 1:
        ...
        for i in range(start.size(0)):
            interpolated = torch.linspace(
                start[i], end[i], n_points, dtype=torch.float64
            )
...
def split_last_dim(
    data: torch.Tensor, split_evenly: bool = True, split_pos: int = None
):
    """
