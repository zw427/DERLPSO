...
def run_single_test(equation, true_params, t_span, dt, estimator_config):
    ...
    trajectories = [
        model._solve_sde(
            true_params, equation.initial_conditions, time_points, seed=42 + i
        )
    ...
    true_params_array = np.array([true_params[name] for name in param_names])[
        np.newaxis, :
    ]

    est_params_dict = dict(zip(param_names, estimated_params_array[0]))
    estimated_data_concatenated = estimator._sde_prediction_concatenated(
        est_params_dict, time_points, seed_base=42
    )

    results = {
        "data": test_data,
        "param": true_params_array,
        "time": test_time,
        "prediction": estimated_params_array,
        "error": true_params_array - estimated_params_array,
        "mse": np.array(
            [np.mean((synthetic_data_concatenated - estimated_data_concatenated) ** 2)]
        ),
    }
