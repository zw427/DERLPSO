...
# FitzHugh_Nagumo
p0 = Parameter([Normal(torch.Tensor([0.7, 0.8]), torch.Tensor([0.5, 0.5]))])

fhn_model = ODE_Models(FitzHugh_Nagumo)
test0 = fhn_model.simulate(100, parameter=p0, init_data=[0, 0], 
...
p1 = Parameter([Normal(torch.Tensor([0.4, 1.3, 1, 1]), torch.Tensor([0.5, 0.5, 0.5, 0.5]))])

lv_model = ODE_Models(Lotka_Volterra)
test1 = lv_model.simulate(100, parameter=p1, init_data=[0.9, 0.9], 
...
p2 = Parameter([Normal(torch.Tensor([2, 4, 1]), torch.Tensor([0.5, 0.5, 0.5]))])

lz_model = ODE_Models(Lorenz)
test2 = lz_model.simulate(100, parameter=p2, init_data=[0, 1, 1.25], 
                   interval=[0, 4], point=5, seed=100)
