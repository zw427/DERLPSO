...
def run_eval(
    func, config, num_data, num_state, parameter, init_data, interval, points, seed
):
    for point in points:
        ...
        if True:
            r2 = model.evaluate(
                MLP(func.num_param, num_state, point, config), test, train
            )
        ...
        if True:
            r3 = model.evaluate(
                RNN(func.num_param, num_state, point, config), test, train
            )
        ...
        if True:
            r4 = model.evaluate(
                ODE_RNN(func.num_param, num_state, point, config), test, train
            )
        ...
        if True:
            r5 = model.evaluate(
                VAE(func.num_param, num_state, point, config), test, train
            )
...
if __name__ == "__main__":
    ...
    run_eval(
        ...
        config="Estimators/configs/fn.yaml",
        num_data=10,
        num_state=2,
        parameter=p0,
        init_data=[0, 0],
    ...
    run_eval(
        func=Lotka_Volterra,
        config="Estimators/configs/lovo.yaml",
        num_data=10,
        num_state=2,
        parameter=p1,
        init_data=[0.9, 0.9],
    ...
    run_eval(
        func=Lorenz,
        config="Estimators/configs/lv.yaml",
        num_data=10,
        num_state=3,
        parameter=p2,
        init_data=[0, 1, 1.25],
